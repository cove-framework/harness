...
> Conforms to the shared design spine (`computeDesign`, `journalDesign`, decisions D-AWS-3/7/8/10/11/15) and the cove THESIS.
...
- The **EC2/Docker sandbox** adapter (D-AWS-15): a per-run container reattached by stable id and driven via SSM Run Command (`docker exec`), replacing `@upstash/box`; MCP per-beat pool in the warm Lambda container.
...
- **G4.1 (hard prerequisite).** Needs `store/ddb.ts` (DocumentClient singleton, conditional-put + atomic-counter helpers), `store/sessionStore.ts`, `store/journalStore.ts`, `store/requestStore.ts`, `store/eventStore.ts`, `store/blobStore.ts` (S3), `store/skillStore.ts`; the 9 DynamoDB tables (Sessions/Requests/Steps/Events/Approvals/Skills/Blobs/Meta/Runs); the S3 bucket; the SSM/Secrets entries for provider creds (no `@upstash/box` token — D-AWS-15); and the **sandbox host** (EC2 instance id + base-image SSM param + the `ssm:SendCommand` grant helper) from `sandbox-stack.ts`. The `store/` layer **is** the replaced Convex adapter seam — every `ctx.runQuery/runMutation` call in the source files below resolves to a `store/` method.
...
- **Downstream:** G4.3 wires these Lambdas as Task states; G4.4 adds the HITL Map; G4.6 consumes the finalized-step Stream + the delta WS frames; G4.7 reuses the dispatch Lambda's warm-container MCP pool + the `mcpDiscover` Lambda.
...
3. `backend/handlers/tasks/dispatchTools.ts` — resolves the **EC2/Docker `SessionEnv`** (D-AWS-15, `docker exec` over SSM), rebuilds executable tools from frozen descriptors, runs `runDispatch` (`dispatch.ts`); child `task()` → **nested `StartExecution.sync`** (replacing the in-Lambda poll loop); MCP pool in the warm container.
...
| [`../../../convex/engine/dispatchTools.ts`](../../../convex/engine/dispatchTools.ts) (`"use node"` action) | `backend/handlers/tasks/dispatchTools.ts` | `resolveSandbox` → **EC2/Docker container** by stable id (`docker exec` over SSM Run Command, D-AWS-15), replacing `@upstash/box`; `runTaskDelegation` poll loop ([`dispatchTools.ts:37-104`](../../../convex/engine/dispatchTools.ts)) **deleted** → nested `StartExecution.sync`. |
...
- **THESIS — tools dispatch OUT-OF-BAND in a self-hosted EC2/Docker sandbox (D-AWS-15).** `dispatchTools.ts` is the only Lambda that touches the sandbox. `setup` resolves-only; `llmStep`/`compact`/`finalize`/`mcpDiscover` never touch it. The two sanctioned non-sandbox executions inside dispatch — `activate_skill` (a `store/skillStore.ts` `GetItem`, never an FS walk) and MCP tools (network) — are preserved ([08 §3](../../design/08-conventions-and-execution-boundary.md), D13). The boundary is identical to the box's; only the adapter changes (`SessionEnv` over `docker exec`/SSM instead of `@upstash/box`).
...
- **§4.2 Action budgets.** `STREAM_DEADLINE_MS≈240_000` force-finalize KEPT in `decode.ts` ([`decode.ts:34, 208-212`](../../../convex/engine/decode.ts)); set the llmStep Lambda timeout to ~5 min (well under the 15-min hard cap). `PER_TOOL_TIMEOUT_MS≈30_000` ([`dispatch.ts:18`](../../../convex/engine/dispatch.ts)) — the EC2/Docker adapter enforces this by wrapping `docker exec` in `timeout 30` and setting the SSM `SendCommand` `ExecutionTimeout` accordingly (was the box `exec timeoutMs=30_000`, [`upstashBox.ts:88`](../../../convex/sandbox/upstashBox.ts)).
...
4. **`dispatchTools` Lambda** (`backend/handlers/tasks/dispatchTools.ts`):
   ...
   - Resolve the sandbox: `ec2DockerBox().createSessionEnv({ id: sandboxName })` where `sandboxName = ${ctx.id}:${instanceId}:${harnessName}` ([08 §3](../../design/08-conventions-and-execution-boundary.md)) — the new adapter (D-AWS-15) implements the **same `SessionEnv` interface** the box did, backed by a Docker container on the G4.1 EC2 host. The stable id is sanitized to a **container name**; `createSessionEnv` ensures the container is running (`docker run -d --name <c>` on first resolve, idempotent `docker start` thereafter), so the same container — and its filesystem — **reattaches across invocations** exactly as the box did. Every `exec`/file op proxies to the host via **`ssm:SendCommand`** (`AWS-RunShellScript` → `docker exec <c> …`); file reads/writes go through `docker exec` (`cat`/`tee`) or a staged `docker cp`. R5 ephemeral-handle semantics map 1:1 onto the Lambda invocation lifetime (the per-action handle cache becomes the per-invocation cache; the container outlives it). **The Lambda needs no inbound networking and stays off-VPC** (SSM is an AWS API call) — D-AWS-11 preserved.
   ...
   - **Container cleanup (D-AWS-15):** on the finalize path (or via an SFN cleanup state), `env.delete()` issues `docker rm -f <c>` (over SSM) so a container does not stay resident forever per `sandboxName`; the host-side age-reaper (G4.1) is the backstop for runs that crash before cleanup (Risks).
...
6. **`finalize` Lambda** (`backend/handlers/tasks/finalize.ts`):
   ...
   - Trigger sandbox cleanup (`env.delete()` → `docker rm -f` for `sandboxName`) here, tied to the SFN execution lifecycle (container-leak risk, D-AWS-15).
...
10. **`compute-stack.ts`** (`backend/cdk/stacks/compute-stack.ts`):
    ...
    - **Reserved concurrency caps** on `dispatchTools` (throttle protection for the single EC2 sandbox host + SSM Run Command throughput + remote MCP under parallel dispatch — D-AWS-15: one host is a shared resource) and `llmStep` (provider rate limits) (D-AWS-11, Risks).
    - Memory: `llmStep`/`dispatchTools` higher (streaming + SSM/Docker round-trips); `setup`/`finalize` minimal.
    - IAM least-privilege per Lambda (task 11).
    - Env/secrets: provider creds (`AI_GATEWAY_API_KEY` or none — AWS chain keyless), the **sandbox host instance id + base-image ref** (D-AWS-15; no `@upstash/box` token), table names, the S3 bucket name, the WS Management API endpoint — from SSM/Secrets Manager (G4.1).
...
11. **IAM** (per Lambda, least-privilege):
    ...
    - `dispatchTools`: `dynamodb:*` on Steps/Sessions/Requests/Skills; `s3:*` on the bucket; `states:StartExecution` + `states:DescribeExecution` for nested child AgentLoop runs; **`ssm:SendCommand` + `ssm:GetCommandInvocation` scoped to the sandbox EC2 instance** (D-AWS-15, drives `docker exec`); outbound HTTPS (MCP).
...
3. **OOB dispatch, sandbox-only.** `dispatchTools` runs tools against the resolved EC2/Docker `SessionEnv` (D-AWS-15); `activate_skill` resolves from `store/skillStore` (no FS), MCP tools reach the network — and `setup`/`llmStep`/`compact`/`finalize` never import the sandbox adapter. A test with a fake SSM/Docker client asserts the adapter satisfies the same `SessionEnv` contract the box did (exec + file ops, stable-id reattach).
...
- **Container leak / single-host saturation (D-AWS-15).** Every distinct `sandboxName` keeps a Docker container resident on the one EC2 host; a crash before cleanup leaks it, and enough concurrent runs exhaust the host's CPU/RAM/disk. **Tie `env.delete()` → `docker rm -f` to the SFN execution lifecycle** (finalize / a cleanup state), rely on the host-side age-reaper (G4.1) as the backstop, and cap `dispatchTools` reserved concurrency to bound concurrent containers. The single host is also a **scale + availability ceiling** — the documented upgrade is Fargate (per-run Firecracker task) behind the same `SessionEnv` seam.
- **SSM Run Command latency (D-AWS-15).** Each `exec`/file op is an `ssm:SendCommand` round-trip (seconds, not the box's sub-second local call); a tool that shells out many times feels it. Acceptable for the simple-for-now posture; batch where possible (one `docker exec sh -c '…'` over several), and the documented lower-latency upgrade is a private exec-agent on the host with `dispatchTools` moved in-VPC.
...
- **No-VPC + outbound HTTPS.** `dispatchTools` (SSM API + MCP) and `mcpDiscover`/MCP reach external HTTPS; off-VPC Lambdas have direct egress. **The SSM-driven sandbox (D-AWS-15) is what keeps `dispatchTools` off-VPC** — it talks to the host through the SSM API, not an inbound socket, so no NAT/ENI penalty. The lower-latency exec-agent upgrade would force the Lambda in-VPC (NAT returns); keep these off-VPC unless that upgrade is genuinely required.
- **Bundle size / dynamic imports.** `ai` + `@ai-sdk/gateway` + `@modelcontextprotocol/sdk` are large (the `@upstash/box` dependency is **removed** — D-AWS-15 — shrinking the `dispatchTools` bundle to the AWS SDK SSM client); keep `mcp/connect.ts`'s SSE transport a lazy dynamic import (code-split) so non-MCP runs don't pay for it, and keep zips under the unzipped 250 MB Lambda limit.
