...
- **IAM baseline + config seam** — per-table least-privilege managed policies the compute stack (G4.2) attaches to Lambda roles; SSM Parameter Store / Secrets Manager parameters for provider keys and channel signing secrets. (No `@upstash/box` token — the sandbox is self-hosted per **D-AWS-15**.)
- **The sandbox host** — a single long-running **EC2 instance running Docker** that replaces `@upstash/box` (**D-AWS-15**): an `sandbox-stack.ts` provisions the instance (Docker via user-data), an instance profile granting **SSM Managed Instance** + ECR pull, a security group with **no inbound** (driven only through SSM Run Command), and a host-side container age-reaper. G4.1 provisions the *host*; the `SessionEnv` adapter that drives `docker exec` over SSM is wired in [G4.2](./phase-g4.2-compute-lambda-actions.md). Exports the instance id + sandbox base-image ref (SSM param) for G4.2.
...
| 3b | **Sandbox host stack** (EC2 + Docker, instance profile, no-inbound SG, reaper) — D-AWS-15 | `backend/cdk/stacks/sandbox-stack.ts` |
...
8. **No durable-loop / HITL / dispatch logic in this phase** (THESIS: AI SDK stays thin; orchestrator owns the loop). G4.1 introduces no control flow — it is pure data + adapter + infra. The loop stays in Step Functions ([G4.3](./phase-g4.3-durable-orchestrator-stepfunctions.md)). The sandbox *host* (EC2/Docker) is **provisioned** here as infra (D-AWS-15, task 7b), but the `SessionEnv` adapter that runs `docker exec` over SSM — and every tool dispatch — is [G4.2](./phase-g4.2-compute-lambda-actions.md); no dispatch code lands in G4.1.
...
7. **SSM / Secrets config seam** — declare `ssm.StringParameter` / `secretsmanager.Secret` placeholders for: provider keys (Anthropic/OpenAI/Google/Gateway) and channel signing secrets (`SLACK_SIGNING_SECRET`, etc. — consumed in [G4.7](./phase-g4.7-channels-workflows-scheduler.md)). **No `@upstash/box` token** (D-AWS-15 — self-hosted sandbox). Also declare the sandbox base-image SSM param + the host instance-id output (task 7b). Export ARNs; a tiny `config.ts` reads them from Lambda env at runtime.
7b. **`sandbox-stack.ts` (D-AWS-15)** — `new ec2.Instance` (Amazon Linux 2023, Docker installed via user-data; size from CDK context, default `t3.medium`); an instance role with `AmazonSSMManagedInstanceCore` (+ ECR read if the sandbox image is in ECR); a security group with **no inbound rules** (the dispatch path reaches it only via `ssm:SendCommand`); user-data that also installs a cron/systemd-timer **container reaper** removing containers idle past a TTL (replaces box `keepAlive` eviction). Export the instance id + an SSM param for the sandbox base image so G4.2's adapter can `docker run`/`docker exec` by name. Grant helper exposed for G4.2 to attach `ssm:SendCommand` + `ssm:GetCommandInvocation` scoped to this instance.
...
- **Sandbox host synths (D-AWS-15).** `cdk synth` produces the `sandbox-stack` with exactly one EC2 instance, an instance profile carrying `AmazonSSMManagedInstanceCore`, a security group with **zero inbound rules**, and the base-image SSM param + instance-id output. The CDK assertion test pins "no inbound" (the off-VPC/SSM-only access posture) and that no `@upstash/box` secret is declared.
