...
`AgentRuntimeConfig` fields the runtime validator accepts (all optional): `profile`, `description`, `model`, `instructions`, `skills`, `tools`, `subagents`, `thinkingLevel`, `compaction`, `durability`, `cwd`, `sandbox`, `mcpServers`, `extensions`. Unknown top-level fields throw. Both `mcpServers` (validated by `assertMcpServers`) and `extensions` (validated by `assertExtensions`) are now first-class optional fields on the agent config and the profile — they are threaded through `resolveAgentProfile`/`extendAgentProfile`. `mcpServers` may *also* be supplied per request (see §2.4); `extensions` entries are a registered extension **name** (string) or an inline `ExtensionFactory` (see §2.4b).
...
- `execute` returns a `Promise<string | ToolResult>`. A plain string is sent back to the LLM verbatim. Thrown errors become tool errors.
...
**Rich results with `ToolResult`.** When a string isn't enough, `execute` may resolve a structured `ToolResult` instead:

```ts
interface ToolResult {
  content: ToolResultContent[];   // what the model sees
  details?: unknown;              // side-channel — NOT sent to the model
  isError?: boolean;             // surfaces as an error tool-result to the model
}
...
`content` is the model-visible payload (text and/or image blocks). `details` is a side-channel for the harness/observability and is never forwarded to the model. `isError: true` surfaces the call as an error tool-result (same shape the model gets on a thrown error). Run *termination* via a result is reserved for the framework's own result tools — user tools cannot terminate the run by returning.

**The user-tool registry (`convex/toolRegistry.ts`).** Like the agent registry, a name-keyed tool registry is **Convex-app-bound** and not exported from the runtime barrel:

```ts
// convex/toolRegistry.ts — scaffolded by `cove init`
defineToolRegistry(map: Record<string, ToolDefinition>): ToolRegistry
registerToolRegistry(registry: ToolRegistry): void
getRegisteredTool(name: string): ToolDefinition | undefined
listRegisteredTools(): string[]
```

The `kind:"user"` tool path is **live**: `dispatchTools` recovers a user tool's `execute` by name via `getRegisteredTool(name)` per isolate (it previously fell through to `errorTool`). Two rules follow:

- User tools must be **module-scope and registered by name** in `convex/toolRegistry.ts`. The frozen plan carries only descriptors; the closure is rebound by name on each action boot.
- Inline-in-`initialize` user tools are **rejected at validation** — there is no name to recover them by across isolates.

`cove build` emits `convex/_cove/toolResolver.ts` (a side-effect import that installs the tool registry per isolate, mirroring the agent/workflow resolvers).

**MCP servers can be declared on the profile/agent config *or* per request.** Both `AgentRuntimeConfig` and `AgentProfile` accept an optional `mcpServers` field (validated by `assertMcpServers`), and the `submitPrompt` mutation + `agentRequests` table accept an optional `mcpServers` array as well. When servers are declared (at either level), a `"use node"` discovery hop under `convex/mcp/` connects to each server and exposes its tools to the model as `mcp__<server>__<tool>`.

### 2.4b Extensions

Extensions are the data-only plugin seam: a registered factory that adds tools, system-prompt fragments, and event handlers, all in a way the replay engine can freeze and re-bind deterministically. Like the agent/tool registries, the extension registry is **Convex-app-bound** (it lives under `convex/`, NOT on the `@cove/runtime` barrel):

```ts
// convex/extensionRegistry.ts — scaffolded by `cove init`
defineExtensionRegistry({ name: factory /* , ... */ }): ExtensionRegistry

type ExtensionFactory = (cove: ExtensionRegistration) => void | Promise<void>;
```

You either register a factory by **name** here, or pass an **inline `ExtensionFactory`** in a profile/agent config's `extensions` array (see below). The factory receives a registration API — note there are **no action methods**, which is exactly what makes re-running a factory per isolate safe:

```ts
interface ExtensionRegistration {
  registerTool(tool: ToolDefinition): void;
  registerSystemPromptFragment(fragment: string): void;
  on(event: ExtensionEventName, handler: (ctx: ExtensionContext, payload) => ...): void;
}

interface ExtensionContext {           // handed to event handlers
  appendEntry(customType: string, data?: unknown): ...;  // writes a `kind:"custom"` entry
  getContextUsage(): ...;
}
```

`appendEntry` writes a `kind:"custom"` session entry — extension side-state that is **excluded from LLM context**, idempotent by deterministic key, and does **not** advance the active leaf.

**Three determinism classes** (each event belongs to exactly one, via `eventClass()` in `extensions/types.ts`):

| Class | When it runs | Constraint | Events |
|---|---|---|---|
| **registration** | once, at load | pure: setup + `registerTool`/`registerSystemPromptFragment` only | `setup` |
| **content-mutation** | behind the `runDecode`/`loadStep` replay guard | pure fn of (frozen plan + persisted step + payload) | `context`, `before_provider_request`, `before_agent_start`, `message_end`, `tool_call`, `tool_result`, `session_before_compact` |
| **notify** | fire-and-forget | skippable on replay; never feeds back into journaled inputs | `agent_start`, `agent_end`, `turn_start`, `turn_end`, `tool_execution_start`, `tool_execution_end`, `session_compact`, `model_select` |

**Which hooks fire today.** Wired and firing (11, plus `appendEntry`): `registerSystemPromptFragment`, `registerTool`, `on('setup')`, `context`, `before_agent_start`, `tool_call`, `tool_result`, `session_before_compact`, `agent_start`, `agent_end`, `turn_end`. Not yet wired (primitives + tests exist; marginal): `tool_execution_start`/`tool_execution_end`, `session_compact`, `turn_start`, `model_select`, `before_provider_request`, `message_end`. Deliberately dropped (interactive/TUI-only, out of scope for the durable backend): `project_trust`, fork/tree, `message_start`/`message_update`, `user_bash`/`user_input`, `registerCommand`/`registerShortcut`/`registerFlag`, `exec`, mid-run `setModel`.

`session_before_compact` (content-mutation): returning **cancel is a NOOP** — the compact step still returns and does **not** fail the run; or supply a `replacementSummary` to skip the summarizer model call entirely.

**The frozen manifest + re-binding.** Registration runs once at load (in the V8 setup path) and is serialized into the run plan as an ordered `extensions` manifest:
...
On replay the engine **re-binds** by re-running the named factories **in manifest order** — order matters because content-mutation handlers form a sequential context-rewrite chain. Inline factories bind by position; only **named** extensions recover their closures across isolates, so anything that must survive a cold action boot should be registered by name.

**Declaring extensions on a profile/agent.** The `extensions` field on `AgentRuntimeConfig`/`AgentProfile` (validated by `assertExtensions`) is an array whose entries are either a registered extension **name** (string) or an inline `ExtensionFactory`:

```ts
export const support = createAgent(() => ({
  model: "anthropic/claude-haiku-4-5",
  extensions: ["audit-log", (cove) => cove.registerSystemPromptFragment("Be terse.")],
}));
```

**Trust model (v1):** extensions are **trusted** — single-tenant per deploy.

**Codegen:** `cove init` scaffolds `convex/extensionRegistry.ts`; `cove build` emits `convex/_cove/extensionResolver.ts` (a side-effect import that installs the registry per isolate, mirroring the agent/tool resolvers).
...
Compaction (`convex/engine/compact.ts` + `src/runtime/compaction.ts`) is **incremental** — it does not re-summarize the whole transcript on every pass. `resolvePreviousCompaction(entryIds, latest)` returns a numeric index into the message list for `prepareCompaction(messages, settings, previous)`. When a prior summary exists, that summary is carried into an **UPDATE prompt** (wrapped in a `<previous-summary>` block) and only the **new slice** since the last compaction is summarized. A split turn is handled by summarizing the **prefix separately**; both summarization calls happen inside the **one journaled action**, and their usage is **summed**. `appendCompactionEntry` accepts and persists an optional `usage`. The `session_before_compact` extension hook fires here (content-mutation): returning **cancel is a NOOP** (the compact step returns; the run does not fail), or a `replacementSummary` short-circuits the summarizer model call.

#### Overflow-then-retry

When the model finishes with a **context-overflow** finish reason (`convex/engine/{loop,decode,steps}.ts`), `decode` marks an **overflow step that appends NO session entry** (`finalizeOverflowStep`). The loop then reacts deterministically:

```
if (decision.overflow) {
  if (compaction enabled && overflowRetries < budget) {
    compact(stepNumber); overflowRetries++; stepNumber++; continue;   // retry on a FRESH step
  } else {
    finalize failed / context_overflow;
  }
}
```

`DEFAULT_OVERFLOW_RETRY_BUDGET = 1`. The counter is **loop-local and deterministic on replay** (same discipline as follow-ups), so the retry re-decodes the freshly-compacted history on a new `stepNumber` rather than mutating the failed step.

#### Observability

The previously silent `catch {}` around `initialize()` in `setup.ts` is now an **observable warning** logged as `event:"setup_initialize_failed"`, so a throwing initializer no longer fails silently.

### Schema as System-of-Record + Reactive Transport (No SSE)

`convex/schema.ts` is the system of record. flue's monolithic `SessionData` blob (an entry *tree* + leaf cursor + task-session refs) is decomposed into **10 relational tables**. `SessionData.version` is now **6**, fronted by a forward-only `migrateSessionData` seam — `fromData` no longer hard-throws on a version mismatch but migrates older snapshots up. Two shared validators sit at the top: `runPlanValidator` (the resolved agent plan snapshotted at setup step 0 — `model`, `systemPrompt`, `tools`, `cwd`, `resultSchema`, `approvalTools`, `compaction`, and `extensions` (the ordered, data-only extension manifest); the replay-determinism backbone) and `usageValidator` (full `PromptUsage` with nested `cost`). The plan is queried back via `getRunPlanContext` and persisted on `sessions.runPlan`. There is **no `overflowRetryBudget`** field on the plan and **no `attempt`** field on step rows.

| # | Table | Purpose |
|---|---|---|
| 1 | `sessions` | one row per session header; tuple `(instanceId, harnessName, sessionName)` is the stable lookup key; holds `leafId`, `taskSessions[]`, `state`, frozen `runPlan` |
| 2 | `sessionEntries` | one row per entry-tree node (`message` \| `compaction` \| `custom`); stored flat, active path rebuilt in app logic; images hoisted to `imageChunks`. `custom` = extension `appendEntry` side-state — excluded from LLM context, idempotent by deterministic key, does **not** advance the active leaf |
...
Provider-specific behavior is factored behind a `ProviderPlugin` layer (`convex/providers/plugin.ts`): `registerProviderPlugin(plugin)`, `getProviderPlugin(providerId)`, and `hasProviderPlugin(providerId)` form a small plugin map, with **four self-registering built-ins** in `convex/providers/builtins.ts`. The capability, registry, thinking, and gateway code now **consult the plugin map first** and fall back to the legacy hardcoded `switch` only when no plugin matches — so the change is **zero behavior** for the built-in providers while opening a clean extension point for new ones. Two related cleanups landed alongside it: `thinking.ts` is now **V8-safe** (the `"use node"` pragma was removed), and `buildBuiltinProviderOptions` was extracted as a shared helper.
  ...
  (verifyThenAdmit:                     3. if supersede: cancelActiveRequests("superseded")
   ...
   [streaming: agentRequestSteps.text                     │     ② plan = step.runMutation(setup.run)
    patched 10-20×/turn via DeltaBatcher] ◀───────────────┤          • getRegisteredAgent(target)→initialize→resolveProfile
                                                          │          • freeze sessions.runPlan (+extension manifest); status=running; emit operation_start
...
3. **The durable run (`agentRun`).** (①) An MCP discovery hop freezes any MCP tools. (②) `setup.run` resolves and **freezes the run plan** onto `sessions.runPlan` — model (`registeredModel ?? request.model ?? session.model ?? "cove-test/mock"`), `systemPrompt` (the `SYSTEM_PREAMBLE` plus instructions), frozen tool *descriptors* (framework/`task`/`activate_skill`/result/MCP — schema only, `execute` rebound per action), derived compaction, `maxSteps`/`maxFollowUps`, and the **ordered extension manifest** (`extensions`). Extension **load + registration runs in this V8 setup path**: the registration-only contract is pure (no action methods), so re-running the named factories on replay rebuilds the same tools/fragments/handlers deterministically. `setup.run` sets `status:"running"` and emits `operation_start`. (③) `runAgentLoop` runs `decode → [HITL] → dispatch → [compact]` over journaled steps; `decode` streams the model and patches `agentRequestSteps.text`/`reasoning` ~10–20×/turn; `dispatch` resolves the sandbox and runs the step's tools (partitioned into `task`, `activate_skill`, and `other`), writing tool-results idempotently. (④) `finalize.run` aggregates usage over `agentRequestSteps`, patches the `agentRequests` terminal fields, and emits `idle`. (⑤) `channels.reply.dispatch` runs (no-op for native/HTTP).
