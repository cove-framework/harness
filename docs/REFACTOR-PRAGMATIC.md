...
- **Add** `overflowRetryBudget: v.optional(v.number())` — the overflow-retry *budget* (revised #4: a budget, not a per-step boolean; the consumed count is loop-local). The step row separately gains an `attempt` discriminator so each overflow attempt is a distinct journal entry.
