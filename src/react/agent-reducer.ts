// Ported from flue · @flue/react · packages/react/src/agent-reducer.ts → @cove/react
// Near-verbatim port. Cove deltas vs flue:
//  - flue's `LlmMessage` ≡ cove's `AgentMessage`, but cove adds `role:'signal'`
//    (messages.ts `SignalMessage`). `reduceMessageBoundary` early-returns on BOTH
//    `role:'signal'` and `role:'toolResult'`; `snapshotMessage`'s param Excludes both.
//  - `@flue/sdk` imports → cove relative imports (../runtime/*, ./ui-types.ts).
//  - content-block field names are identical to flue (text.text, thinking.thinking,
//    toolCall.{id,name,arguments}, image.{data,mimeType}).

import type { AgentMessage } from "../runtime/messages.ts";
import { IMAGE_DATA_OMITTED } from "../runtime/event-image.ts";
import type { AttachedAgentEvent, PromptUsage } from "../runtime/types.ts";
import type { AgentPromptImage, UIMessage, UIMessagePart } from "./ui-types.ts";
...
function reduceMessageBoundary(
	state: AgentState,
	event: StreamAgentEvent & { message: AgentMessage },
): AgentState {
	// cove delta: `role:'toolResult'` AND `role:'signal'` carry no UI-renderable message.
	if (event.message.role === "toolResult" || event.message.role === "signal") return state;
...
function snapshotMessage(
	id: string,
	message: Exclude<AgentMessage, { role: "toolResult" } | { role: "signal" }>,
	done: boolean,
	previous?: UIMessage,
): UIMessage {
