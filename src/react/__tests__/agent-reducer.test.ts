// Ported from flue · @flue/react · test/agent-reducer.test.ts → @cove/react
// All 14 deterministic reducer cases retargeted to cove types (`AttachedAgentEvent` from
// ../../runtime/types.ts, `AgentMessage` from ../../runtime/messages.ts) + ONE new
// `role:'signal'` guard case proving a `message_*` event carrying a signal-role
// `AgentMessage` returns the reducer state unchanged.
//
// The reducer only reads `message.role` and `message.content`; cove's `AgentMessage`
// adds required provider/usage/timestamp fields the reducer never touches, so test
// fixtures supply the minimal `{ role, content }` shape cast to `AgentMessage`.

import { describe, expect, it } from "vitest";
import type { AgentMessage } from "../../runtime/messages.ts";
import type { AttachedAgentEvent } from "../../runtime/types.ts";
import { emptyAgentState, reduceAgentEvent } from "../agent-reducer.ts";
...
/** Minimal message fixture cast to AgentMessage (reducer reads only role + content). */
function msg(value: { role: string; content: unknown }): AgentMessage {
...
function message(
	...
	return { ...base, type, message: msg(value), eventIndex: 1, ...extra } as AttachedAgentEvent & {
		submissionId?: string;
	};
}
...
describe("reduceAgentEvent()", () => {
	...
	it("preserves a late-stream tool result through terminal reconciliation", () => {
		...
		state = reduceAgentEvent(
			...
			message(
				...
				{
					role: "assistant",
					content: [{ type: "toolCall", id: "tool-1", name: "search", arguments: { q: "cove" } }],
				},
	...
	it("keeps another local submission pending when one submission becomes idle", () => {
		...
		state = reduceAgentEvent(state, {
			...
			eventIndex: 10,
			submissionId: "submission-1",
		} as AttachedAgentEvent & { submissionId?: string });
	...
	// New (cove-specific): a message_* event carrying a role:'signal' AgentMessage must
	// return the reducer state unchanged — no malformed UIMessage.
	it("returns state unchanged for a message_* event carrying a role:'signal' message", () => {
		const signalStart = message(
			"message_start",
			{
				role: "signal",
			...
			{ turnId: "turn-1", eventIndex: 1 },
		);
		const afterStart = reduceAgentEvent(emptyAgentState, signalStart);
		expect(afterStart).toBe(emptyAgentState);
		expect(afterStart.messages).toEqual([]);

		const signalEnd = message(
			"message_end",
			{
				role: "signal",
				type: "stream_continued",
			...
			{ turnId: "turn-1", eventIndex: 2 },
		);
		const afterEnd = reduceAgentEvent(afterStart, signalEnd);
		expect(afterEnd).toBe(afterStart);
		expect(afterEnd.messages).toEqual([]);
	});
});
