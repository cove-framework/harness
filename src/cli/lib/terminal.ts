// Ported verbatim from flue · @flue/cli · packages/cli/src/lib/terminal.ts → @cove/cli.
// New (Convex backend). The only behavioral change vs. the flue source is the `flue`→`cove` brand word
// in `success`. The `[cove]`-branded diagnostics surface every other CLI piece uses (terminal helpers:
// error/success/note/dim/blue/brand/brandRows/row/section). Pure Node — no Convex, no box.
