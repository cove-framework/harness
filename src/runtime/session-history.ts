// Ported from flue · @flue/runtime · packages/runtime/src/session-history.ts
// (pi message imports → ./messages.ts; SignalMessage moved to ./messages.ts).
...
export class SessionHistory {
	 ...
	 *
	 * - `afterLeafId === null` returns the entire active path.
	 * - When the id is found, returns entries strictly after it.
	 * - When the id is *not* on the current active path (a branch switch happened
	 *   mid-window), returns `[]` — bounded windowing prefers a loud zero over
	 *   silent overcounting.
	 */
	...
	appendCompaction(input: CompactionAppendInput): string {
		if (!this.byId.has(input.firstKeptEntryId)) {
			throw new Error(`[cove] Cannot compact: entry "${input.firstKeptEntryId}" does not exist.`);
		}
	...
	setLeaf(entryId: string): void {
		if (!this.byId.has(entryId)) {
			throw new Error(`[cove] Cannot set leaf: entry "${entryId}" does not exist.`);
		}
		this.leafId = entryId;
	}
