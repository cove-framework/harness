// Ported verbatim from flue · @flue/runtime · packages/runtime/src/skill-frontmatter.ts
...
export function parseSkillMarkdown(
	...
	if (!match) {
		throw new Error(
			`[cove] Skill ${options.path} is missing YAML frontmatter. Start SKILL.md with "---", include "name" and "description", then close the block with "---".`,
		);
	}
	...
	try {
		...
		throw new Error(`[cove] Skill ${options.path} has invalid YAML frontmatter.${detail}`);
	}
	if (!isRecord(raw)) {
		throw new Error(`[cove] Skill ${options.path} frontmatter must be a YAML mapping.`);
	}
	...
	if ([...description].length > 1024) {
		throw new Error(
			`[cove] Skill ${options.path} frontmatter description exceeds the 1024-character Agent Skills limit. Shorten "description" to a concise one-line summary.`,
		);
	}
	...
	if (compatibility !== undefined && [...compatibility].length > 500) {
		throw new Error(`[cove] Skill ${options.path} compatibility must be at most 500 characters.`);
	}
...
function validateSkillName(name: string, options: ParseSkillMarkdownOptions): void {
	...
	if ([...normalized].length > 64) {
		throw new Error(`[cove] Skill ${options.path} name must be at most 64 characters.`);
	}
	if (!/^[\p{L}\p{N}-]+$/u.test(normalized) || normalized !== normalized.toLowerCase()) {
		throw new Error(
			`[cove] Skill ${options.path} frontmatter name "${name}" must contain only lowercase letters, numbers, and hyphens. Use a spec-compliant value such as "review-pr".`,
		);
	}
	if (normalized.startsWith("-") || normalized.endsWith("-") || normalized.includes("--")) {
		throw new Error(
			`[cove] Skill ${options.path} frontmatter name "${name}" must not start or end with a hyphen or contain consecutive hyphens. Use a spec-compliant value such as "review-pr".`,
		);
	}
	if (normalized !== options.directoryName.normalize("NFKC")) {
		throw new Error(
			`[cove] Skill ${options.path} declares frontmatter name "${name}", but Agent Skills requires it to match directory "${options.directoryName}"; names must match. Rename the directory or change "name" so they match.`,
		);
	}
}
...
function requireString(value: unknown, path: string, field: string): string {
	if (typeof value !== "string" || value.trim().length === 0) {
		throw new Error(`[cove] Skill ${path} must define frontmatter ${field} as a non-empty string.`);
	}
	return value.trim();
}
...
function optionalString(value: unknown, path: string, field: string): string | undefined {
	...
	if (typeof value !== "string") {
		throw new Error(`[cove] Skill ${path} frontmatter ${field} must be a string when provided.`);
	}
...
function parseMetadata(value: unknown, path: string): Record<string, string> | undefined {
	if (value === undefined || value === null) return undefined;
	if (!isRecord(value)) {
		throw new Error(`[cove] Skill ${path} frontmatter metadata must be a string-to-string mapping.`);
	}
	const entries = Object.entries(value).map(([key, metadataValue]) => {
		...
		if (typeof metadataValue !== "string") {
			throw new Error(
				`[cove] Skill ${path} frontmatter metadata must be a string-to-string mapping.`,
			);
		}
...
function parseAllowedTools(value: unknown, path: string): string[] | undefined {
	if (value === undefined || value === null) return undefined;
	if (typeof value !== "string") {
		throw new Error(`[cove] Skill ${path} frontmatter allowed-tools must be a string when provided.`);
	}
