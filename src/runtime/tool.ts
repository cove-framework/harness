// Ported from flue · @flue/runtime · packages/runtime/src/tool.ts (errors import → ./errors.ts).
 ...
 * unchanged. Tool names are checked for collisions with other active tools when
 * a session assembles its tool list.
 */
export function defineTool<TParams extends ToolParameters>(
	tool: ToolDefinition<TParams>,
): ToolDefinition {
	if (!tool || typeof tool !== "object") {
		throw new Error("[cove] defineTool() requires a tool definition object.");
	}
	if (typeof tool.name !== "string" || tool.name.trim().length === 0) {
		throw new Error("[cove] defineTool({ name }) must be a non-empty string.");
	}
	if (typeof tool.description !== "string" || tool.description.trim().length === 0) {
		throw new Error("[cove] defineTool({ description }) must be a non-empty string.");
	}
	if (!tool.parameters || typeof tool.parameters !== "object") {
		throw new Error("[cove] defineTool({ parameters }) is required.");
	}
	if (typeof tool.execute !== "function") {
		throw new Error("[cove] defineTool({ execute }) must be a function.");
	}
 ...
 * assemblies reuse one stable JSON Schema object. The agent loop caches compiled
 * argument validators by schema object identity, so handing it a fresh
 * conversion per turn would defeat that cache.
 */
 ...
 *   {@link ToolInputValidationError}, which the agent loop surfaces to the model
 *   as an error tool-result so it can self-correct.
 *
 * Definitions with raw JSON Schema `parameters` (including everything already
 * normalized here) are returned unchanged. Idempotent.
 */
export function normalizeToolDefinition(tool: ToolDefinition): ToolDefinition {
	...
	if (!isTopLevelObjectSchema(schema)) {
		throw new Error(
			`[cove] Tool "${tool.name}" parameters must be a top-level object schema ` +
